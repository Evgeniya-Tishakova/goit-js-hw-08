...
const imageCardTemplate = images.map((el) => createGalleryEl(el)).join("");

const galleryListEl = document.querySelector(".js-gallery");
galleryListEl.innerHTML = imageCardTemplate;
...
galleryListEl.addEventListener("click", (event) => {
  ...
  if (clickedEl.nodeName === "A") {
    alert("Завантаження зображення заборонено.");
    return;
  }
    ...
    const modalWindowInstance = basicLightbox.create(`
      <div class="modal">
        <img
          ...
          src="${clickedEl.getAttribute("data-source")}"
          alt="${clickedEl.alt}"
        />
      </div>
    `);
